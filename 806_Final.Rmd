...
|     I conducted analysis of lesions in ImageJ (Schneider et al., 2012). I measured the scaling tack in each photograph (generally with a scale of 18 pixels/mm) using the line tool. Then, I traced the lesion using the freehand tool and measured its area to the nearest 0.000. 

# Results 

[data compiled as means of each inoc/season/tree -> averaging 3 subsamples per study unit for analysis]

[notes for me: 
#Dunnetts - comparing to "control"
#install.packages("DescTools")
#library(DescTools)
DunnettTest(x = data$Meas, g = data$Trt)

#HSD Test
HSD.test(model, "Trt", console = T)
HSD.test(model, "Site", console = T)
HSD.test(model, "Inoc", console = T)
HSD.test(model, "HT", console = T)

example lsd ranking:
seednotmod<-lm(Meas ~ Till + Block + Till:Block 
               + Seed + Seed:Block + Till:Seed, seedat)
anova(seednotmod)
#Till:Block MS = 62.9, DF = 9
#Seed:Block MS = 63.7, DF = 9
library(agricolae)
#install.packages("agricolae")
till_comp<-LSD.test(seedat$Meas, seedat$Till, DFerror = 9,
                    MSerror = 62.9)
till_comp ->gives ranked (eg. separated results)
seed_comp<-LSD.test(seedat$Meas, seedat$Seed, DFerror = 9,
                    MSerror = 63.7)
seed_comp]
